...
function assertSourceUrl(sourceUrl: string, sourceType: string, allowedOfficialHosts: Set<string>): URL {
	...
	if (BLOCKED_SEARCH_HOSTS.has(hostname)) fail(`Search-result hosts are not allowed for assets: ${sourceUrl}`);
	if (sourceType === 'store') {
		...
		if (!officialHost) fail(`official and press-kit assets must match assetBootstrap.allowedHosts: ${sourceUrl}`);
	}
	return parsed;
}
...
async function main() {
	...
	for (const [index, asset] of candidates.entries()) {
		...
		if (!isWithin(inputAssetsDir, destination)) fail(`${id} source must stay under site-input/assets/: ${source}`);
