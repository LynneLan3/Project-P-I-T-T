...
test('formal homepage: hub URL uses Experience Homepage with full data', { timeout: 240_000 }, () => {
	...
	try {
		...
		// Featured spread (featured:true guide with cover) is a full-card link; the image is visual only.
		assert.match(html, /<a class="exp-featured" href="\/example-guide\/"[\s\S]*?class="exp-featured__image"><img/);
