...
- Awareness that the Shift Scheduler is part of Automation → Shift Scheduler → x300 Combo flow; use neighboring tools like the [Trade Terminal]({{page:trade-terminal}}) or [Panel Rack]({{page:panel-rack}}) for supporting automation tasks.
...
4. If you need supporting tools, consult [Panel Rack]({{page:panel-rack}}) for signal routing and the [Trade Terminal]({{page:trade-terminal}}) for interacting with item flows outside the lane.
